...
export default class Worker {
    ...
    static async Add ( fields ) {
        try {
            ...
            let collection = mongoClient.collection('medical_checkup')
            await collection.insertOne(fields)
            return fields

        } catch (err) {
            console.log(err)
            throw ({...{err: 7001000, msg: 'CMedicalCheckup Add'}, ...err})
        }
    }
    ...
    static async GetById ( ids ) {
        try {
            ...
            console.log(err)
            throw ({...{err: 7001000, msg: 'CMedicalCheckup GetById'}, ...err})
        }
    }
    ...
    static async Get ( fields ) {
        try {
            ...
            if (!fields.specialist_ids && !fields.specialist_ids)
                arAggregate.push({
                    $sort: {
            ...
            if (fields.specialist_ids || fields.research_ids) arAggregate[0].$match = {
                $or: [
                    { specialist_ids: { $in: fields.specialist_ids ? fields.specialist_ids : [] }},
                    { research_ids: { $in: fields.research_ids ? fields.research_ids : [] }},
                ]}
            //if (fields.specialist_ids || fields.research_ids) arAggregate[0].$match.specialist_ids = { $in: fields.specialist_ids }
            ...
            console.log(err)
            throw ({...{err: 7001000, msg: 'CMedicalCheckup Get'}, ...err})
        }
    }
    ...
    static async Edit ( id, fields ) {
        try {
            ...
            console.log(err)
            throw ({code: 8001000, msg: 'CMedicalCheckup Edit'})
        }
    }
}
